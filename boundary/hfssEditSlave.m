% ----------------------------------------------------------------------------
% 20200720 newFunction 功能: 更新从边界条件的位置、方向及入射角
%
% Copyright 2020, Vega Lau (vegalau@qq.com)
% ----------------------------------------------------------------------------
function hfssEditSlave(fid, Name, Start, End, Units, MasterName, Theta)
...
fprintf(fid, 'false, _\n');
fprintf(fid, '"Master:=", _\n');  
fprintf(fid, '"%s", _\n', MasterName);
fprintf(fid, '"UseScanAngles:=", _\n');
fprintf(fid, 'true, _\n');
fprintf(fid, '"Phi:=", _\n');
fprintf(fid, '"0deg", _\n');
fprintf(fid, '"Theta:=", _\n');
